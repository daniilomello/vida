...
describe("deleteCard", () => {
  ...
  it("should soft-delete a card and return 200 with message", async () => {
    ...
    expect(JSON.parse(res.body)).toEqual({ message: "Card deactivated" });
  });
  ...
  it("should throw 401 when principalId is missing", async () => {
    await expect(
      handler(makeEvent({ id: "card-uuid-1" }, ""), {} as never, () => {}),
    ).rejects.toMatchObject({ statusCode: 401 });
  });

  it("should call DynamoDB GetCommand then UpdateCommand setting active=false", async () => {
    mockSend.mockResolvedValueOnce({ Item: existingItem }).mockResolvedValueOnce({});
    ...
    expect(updateCmd.input.ExpressionAttributeValues[":active"]).toBe(false);
  });
});
